...
#   annual_error[j,:,:] = 100*np.fabs(np.sum(obs_runoff, axis=0) - np.sum(vic_runoff[j,:,:,:], axis=0))/np.sum(obs_runoff, axis=0)

#   for m in xrange(0, 12):
#     monthly_error[j,m,:,:] = 100*np.fabs(obs_runoff[m,:,:] - vic_runoff[j,m,:,:])/obs_runoff[m,:,:]
